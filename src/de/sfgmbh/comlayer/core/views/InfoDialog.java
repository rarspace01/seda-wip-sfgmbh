...
public class InfoDialog extends JDialog implements IntfAppObserver {
	...
	/**
	 * Create the dialog with a custom text, title and settings The settings
	 * (like a custom logo or buttons) are set depending on the submitted
	 * variant string. Supported variant strings are:<br>
	 * "success", "info", "warn" and "error" where "error" is the default
	 * variant.
	 * 
	 * @param text
	 * @param title
		...
		if (variant.equals("success")) {
			this.getLblIcon()
					.setIcon(
							new ImageIcon(
									InfoDialog.class
											.getResource("/de/sfgmbh/comlayer/core/images/Success.png")));
		} else if (variant.equals("info")) {
			this.getLblIcon()
					.setIcon(
							new ImageIcon(
									InfoDialog.class
											.getResource("/de/sfgmbh/comlayer/core/images/InfoDialogIcon.png")));
		} else if (variant.equals("warn")) {
			this.getLblIcon()
					.setIcon(
							new ImageIcon(
									InfoDialog.class
											.getResource("/de/sfgmbh/comlayer/core/images/warning.png")));
		} else if (variant.equals("error")) {
			this.getLblIcon()
					.setIcon(
							new ImageIcon(
									InfoDialog.class
											.getResource("/de/sfgmbh/comlayer/core/images/error.png")));
		}
		createContents(title);
	}
	...
	private void createContents(String title) {
		...
		getContentPane()
				.setLayout(
						new MigLayout(
								"",
								"[5px:n][88.00,center][5px:n][250px:n:500px,center][10px:n:10px]",
								"[100px:n:400px,center][40px:n:40px,center]"));
		getContentPane().add(getLblIcon(),
	...
	private JLabel getLblIcon() {
		if (lblIcon_ == null) {
			lblIcon_ = new JLabel("");
			lblIcon_.setIcon(new ImageIcon(InfoDialog.class
					.getResource("/de/sfgmbh/comlayer/core/images/error.png")));
	...
	@Override
	public void change() {
		String msg = AppModel.getInstance().getExceptionHandler()
				.getExceptionMsg_();
		String title = AppModel.getInstance().getExceptionHandler()
				.getExceptionTitle_();
		String variant = AppModel.getInstance().getExceptionHandler()
				.getExceptionVariante_();
