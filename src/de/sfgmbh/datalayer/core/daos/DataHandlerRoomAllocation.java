...
public class DataHandlerRoomAllocation implements IntfDataObservable,
		IntfDataFilter, IntfDataRoomAllocation {
	...
	public List<IntfRoomAllocation> getAll() {
		...
		try {
			...
			DataModel
					...
					.setNewException(
							("Es ist ein SQL-Fehler (DataHandlerRoomAllocation-01) aufgetreten:<br /><br />" + e
			...
			DataModel
					...
					.getExceptionsHandler()
					.setNewException(
							("Es ist ein unbekannter Fehler (DataHandlerRoomAllocation-02) in der Datenhaltung aufgetreten:<br /><br />" + e
									.toString()), "Fehler!");
		} finally {
			dataManager.dispose();
		}
	 ...
	 * 
	 * @see
	 * de.sfgmbh.datalayer.core.daos.IntfDataRoomAllocation#getByFilter(java
	 * .util.HashMap)
	 */
	@Override
	public List<IntfRoomAllocation> getByFilter(HashMap<String, String> filter) {
		...
		if (filter.containsKey("status")
				&& filter.get("status").equals("wartend")) {
			filter.put("status", "waiting");
		}
		if (filter.containsKey("status")
				&& filter.get("status").equals("freigegeben")) {
			filter.put("status", "accepted");
		}
		if (filter.containsKey("status")
				&& filter.get("status").equals("abgelehnt")) {
			filter.put("status", "denied");
		}
		if (filter.containsKey("status")
				&& filter.get("status").equals("Gegenvorschlag")) {
		...
		try {
			if (filterDm == null) {
				...
				if (filter.containsKey("courseenabled")) {
					filterDm.prepare("SELECT public.roomallocation.*, public.course.*, public.user.*, public.room.*, public.chair.* "
							+ "FROM public.roomallocation, public.room, public.course, public.user, public.chair, public.lecturer "
							+ "WHERE public.roomallocation.courseid = public.course.courseid "
							+ "AND public.course.lecturerid = public.user.userid "
							+ "AND public.course.lecturerid = public.lecturer.userid "
			...
			if (filter.containsKey("lecturer")
					&& filter.get("lecturer") != null
					&& filter.get("lecturer") != ""
					&& filter.get("lecturer") != "<alle>") {
			...
			if (filter.containsKey("course") && filter.get("course") != null
					&& filter.get("course") != ""
					&& filter.get("course") != "<alle>") {
			...
			if (filter.containsKey("semester")
					&& filter.get("semester") != null
					&& filter.get("semester") != ""
					&& filter.get("semester") != "<alle>") {
			...
			if (filter.containsKey("login") && filter.get("login") != null
					&& filter.get("login") != ""
					&& filter.get("login") != "<alle>") {
				filterDm.getPreparedStatement().setString(10,
						filter.get("login"));
			} else {
				filterDm.getPreparedStatement().setString(10, "%");
			}
			if (filter.containsKey("roomid") && filter.get("roomid") != null
					&& filter.get("roomid") != ""
					&& filter.get("roomid") != "<alle>") {
				filterDm.getPreparedStatement().setInt(11,
						Integer.parseInt(filter.get("roomid")));
				filterDm.getPreparedStatement().setInt(12,
						Integer.parseInt(filter.get("roomid")));
			} else {
				filterDm.getPreparedStatement().setInt(11, 0);
				filterDm.getPreparedStatement().setInt(12, 2147483647);
			}
			if (filter.containsKey("chairid") && filter.get("chairid") != null
					&& filter.get("chairid") != ""
					&& filter.get("chairid") != "<alle>") {
				filterDm.getPreparedStatement().setInt(13,
						Integer.parseInt(filter.get("chairid")));
				filterDm.getPreparedStatement().setInt(14,
						Integer.parseInt(filter.get("chairid")));
			} else {
				filterDm.getPreparedStatement().setInt(13, 0);
				filterDm.getPreparedStatement().setInt(14, 2147483647);
			}
			if (filter.containsKey("day") && filter.get("day") != null
					&& filter.get("day") != "" && filter.get("day") != "<alle>") {
				filterDm.getPreparedStatement().setInt(15,
						Integer.parseInt(filter.get("day")));
				filterDm.getPreparedStatement().setInt(16,
						Integer.parseInt(filter.get("day")));
			} else {
			...
			if (filter.containsKey("courseenabled")
					&& filter.get("courseenabled") != null
					&& filter.get("courseenabled") != ""
					&& filter.get("courseenabled") != "<alle>") {
			...
			DataModel
					...
					.getExceptionsHandler()
					.setNewException(
							("Es ist ein SQL-Fehler aufgetreten.<br /><br />Fehler DataHandlerRoomAllocation-17:<br />" + e
									.toString()), "Datenbank-Fehler!");
		} catch (Exception e) {
			...
			DataModel
					...
					.getExceptionsHandler()
					.setNewException(
							("Es ist ein unbekannter Fehler in der Datenhaltung aufgetreten:<br /><br />Fehler DataHandlerRoomAllocation-18:<br />" + e
									.toString()), "Fehler!");
		} finally {
			filterDm.dispose();
		}
	 ...
	 * 
	 * @see
	 * de.sfgmbh.datalayer.core.daos.IntfDataRoomAllocation#getConflictingAllocation
	...
	public List<IntfRoomAllocation> getConflictingAllocation(
		...
		try {
			if (conflictingAllocationDm == null) {
				...
				conflictingAllocationDm
						.prepare("SELECT public.roomallocation.*, public.course.*, public.user.*, public.room.*, public.chair.* "
								+ "FROM public.roomallocation, public.room, public.course, public.user, public.chair, public.lecturer "
								+ "WHERE public.roomallocation.courseid = public.course.courseid "
								+ "AND public.course.lecturerid = public.user.userid "
								+ "AND public.course.lecturerid = public.lecturer.userid "
			...
			conflictingAllocationDm.getPreparedStatement().setInt(1,
					roomAllocation.getRoom_().getRoomId_());
			conflictingAllocationDm.getPreparedStatement().setInt(2,
					roomAllocation.getDay_());
			conflictingAllocationDm.getPreparedStatement().setInt(3,
					roomAllocation.getTime_());
			conflictingAllocationDm.getPreparedStatement().setString(4,
					roomAllocation.getSemester_());
			conflictingAllocationDm.getPreparedStatement().setInt(5,
					roomAllocation.getRoomAllocationId_());

			ResultSet resultSet = conflictingAllocationDm
					.selectPreparedStatement();
			while (resultSet.next()) {
				listRoomAllocation.add(this.makeRoomAllocation(resultSet,
						"conflictingChildObject"));
			}
		} catch (SQLException e) {
			...
			DataModel
					...
					.getExceptionsHandler()
					.setNewException(
							("Es ist ein SQL-Fehler aufgetreten.<br /><br />Fehler DataHandlerRoomAllocation-15:<br />" + e
									.toString()), "Datenbank-Fehler!");
		} catch (Exception e) {
			...
			DataModel
					...
					.getExceptionsHandler()
					.setNewException(
							("Es ist ein unbekannter Fehler in der Datenhaltung aufgetreten:<br /><br />Fehler DataHandlerRoomAllocation-16:<br />" + e
									.toString()), "Fehler!");
		} finally {
	 ...
	 * 
	 * @see
	 * de.sfgmbh.datalayer.core.daos.IntfDataRoomAllocation#getConflictingDates
	 * ()
	 */
	@Override
	 ...
	 * 
	 * @see
	 * de.sfgmbh.datalayer.core.daos.IntfDataRoomAllocation#setConflicts(java
	 * .util.List)
	 */
	@Override
	public List<IntfRoomAllocation> setConflicts(
			List<IntfRoomAllocation> listRoomAllocation) {
		List<HashMap<String, Object>> conflictingDates = this
				.getConflictingDates();
		List<IntfRoomAllocation> setAllocations = new ArrayList<IntfRoomAllocation>();

		// Iterate over all room allocations and check for each allocation all
		// conflicting dates
		for (IntfRoomAllocation roomAllocation : listRoomAllocation) {
			for (HashMap<String, Object> conflict : conflictingDates) {
				if (roomAllocation.getRoom_().getRoomId_() == (int) conflict
						.get("roomid")
						&& roomAllocation.getDay_() == (int) conflict
								.get("day")
						&& roomAllocation.getTime_() == (int) conflict
								.get("time")
						&& roomAllocation.getSemester_().equals(
								(String) conflict.get("semester"))) {
	...
	public IntfRoomAllocation get(int id) {
		...
		try {
			...
			while (resultSet.next()) {
				IntfRoomAllocation roomAllocation = this.makeRoomAllocation(
						resultSet, "normal");
				for (HashMap<String, Object> conflict : this
						.getConflictingDates()) {
					if (roomAllocation.getRoom_().getRoomId_() == (int) conflict
							.get("roomid")
							&& roomAllocation.getDay_() == (int) conflict
			...
			DataModel
					...
					.getExceptionsHandler()
					.setNewException(
							("Es ist ein SQL-Fehler aufgetreten.<br /><br />Fehler DataHandlerRoomAllocation-20:<br />" + e
									.toString()), "Datenbank-Fehler!");
		} catch (Exception e) {
			...
			DataModel
					...
					.getExceptionsHandler()
					.setNewException(
							("Es ist ein unbekannter Fehler in der Datenhaltung aufgetreten:<br /><br />Fehler DataHandlerRoomAllocation-21:<br />" + e
									.toString()), "Fehler!");
		} finally {
			dataManager.dispose();
		}
	 ...
	 * 
	 * @see
	 * de.sfgmbh.datalayer.core.daos.IntfDataRoomAllocation#save(de.sfgmbh.applayer
	 * .core.model.RoomAllocation)
	 */
	@Override
	public boolean save(IntfRoomAllocation roomAllocation) {
		...
		if (roomAllocation.getRoomAllocationId_() == -1) {
			try {
				...
				dataManager
						.prepare("INSERT INTO public.roomallocation"
								+ "(courseid, roomid, semester, day, time, approved, orgamessage, comment)"
								+ "VALUES (?,?,?,?,?,?,?,?)");
				dataManager.getPreparedStatement().setInt(1,
						roomAllocation.getCourse_().getCourseId_());
				dataManager.getPreparedStatement().setInt(2,
						roomAllocation.getRoom_().getRoomId_());
				dataManager.getPreparedStatement().setString(3,
						roomAllocation.getSemester_());
				dataManager.getPreparedStatement().setInt(4,
						roomAllocation.getDay_());
				dataManager.getPreparedStatement().setInt(5,
						roomAllocation.getTime_());
				dataManager.getPreparedStatement().setString(6,
						roomAllocation.getApproved_());
				dataManager.getPreparedStatement().setString(7,
						roomAllocation.getOrgaMessage_());
				dataManager.getPreparedStatement().setString(8,
						roomAllocation.getComment_());
				...
				DataModel
						...
						.getExceptionsHandler()
						.setNewException(
								("Es ist ein SQL-Fehler aufgetreten:<br /><br />Fehler DataHandlerRoomAllocation-22:<br />" + e
										.toString()), "Datenbank-Fehler!");
			} catch (Exception e) {
				...
				DataModel
						...
						.getExceptionsHandler()
						.setNewException(
								("Es ist ein unbekannter Fehler in der Datenhaltung aufgetreten:<br /><br />Fehler DataHandlerRoomAllocation-22:<br />" + e
										.toString()), "Fehler!");
			} finally {
				dataManager.dispose();
			}
			...
			try {
				...
				dataManager
						.prepare("UPDATE public.roomallocation SET "
								+ "courseid = ?, roomid = ?, semester = ?, day = ?, time = ?, approved = ?, orgamessage = ?, comment = ? "
				...
				DataModel
						...
						.getExceptionsHandler()
						.setNewException(
								("Es ist ein SQL-Fehler aufgetreten:<br /><br />Fehler DataHandlerRoomAllocation-23:<br />" + e
										.toString()), "Datenbank-Fehler!");
			} catch (Exception e) {
				...
				DataModel
						...
						.getExceptionsHandler()
						.setNewException(
								("Es ist ein unbekannter Fehler in der Datenhaltung aufgetreten:<br /><br />Fehler DataHandlerRoomAllocation-24:<br />" + e
										.toString()), "Fehler!");
			} finally {
				dataManager.dispose();
			}
	...
	private RoomAllocation makeRoomAllocation(ResultSet resultSet,
			String variant) {
		...
		try {
			...
			returnRoomAllocation.setRoomAllocationId_(resultSet
					.getInt("roomallocationid"));
			returnRoomAllocation.setCourse_(DataModel.getInstance()
					.getDataHandlerCourse().makeCourse(resultSet));
			returnRoomAllocation.setRoom_(DataModel.getInstance()
					.getDataHandlerRoom().makeRoom(resultSet));
			...
			returnRoomAllocation.setOrgaMessage_(resultSet
					.getString("orgamessage"));
			returnRoomAllocation.setComment_(resultSet.getString("comment"));
			/*
			 * Do net set conflicting allocations here as it causes a lot of SQL
			 * queries when building lists - use
			 * setConflicts(List<RoomAllocation> roomAllocations) instead //
			 * Avoid loops if (variant.equals("conflictingChildObject")) { //
			...
			DataModel
					...
					.getExceptionsHandler()
					.setNewException(
							("Es ist ein SQL-Fehler (DataHandlerRoomAllocation-03) aufgetreten:<br /><br />" + e
									.toString()), "Datenbank-Fehler!");
		} catch (Exception e) {
			...
			DataModel
					...
					.getExceptionsHandler()
					.setNewException(
							("Es ist ein unbekannter Fehler (DataHandlerRoomAllocation-04) in der Datenhaltung aufgetreten:<br /><br />" + e
									.toString()), "Fehler!");
		}
	 ...
	 * 
	 * @see
	 * de.sfgmbh.datalayer.core.daos.IntfDataRoomAllocation#register(de.sfgmbh
	...
	public void register(IntfDataObserver observer) {
		if (observer instanceof IntfDataObserver) {
			...
			DataModel
					...
					.getExceptionsHandler()
					.setNewException(
							"Das Objekt implementiert nicht das Observer-Interface und kann daher nicht hinzugefügt werden!<br />Fehler: DataHandlerRoomAllocation-12",
							"Fehler!");
		}
	}
	 ...
	 * 
	 * @see
	 * de.sfgmbh.datalayer.core.daos.IntfDataRoomAllocation#unregister(de.sfgmbh
	 * .datalayer.core.definitions.IntfDataObserver)
	 */
	@Override
