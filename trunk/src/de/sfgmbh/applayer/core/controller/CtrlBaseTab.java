...
public class CtrlBaseTab implements IntfCtrlBaseTab {
	 ...
	 * 
	 * @see
	 * de.sfgmbh.applayer.core.controller.IntfCtrlBaseTab#login(java.lang.String
	 * , java.lang.String)
	 */
	@Override
	public IntfUser login(String login, String pw) {
			...
			if (checkUser.checkPassword(pw) == true) {
				...
				if (checkUser.isDisabled_()) {
					AppModel.getInstance()
							.getExceptionHandler()
							.setNewException(
									"Ihr Benutzerkonto ist gesperrt.<br />Für genauere Informationen wenden Sie sich bitte an die Verwaltung.",
									"Fehler!");
					return null;
				}
				...
				checkUser
						.setLastLogin_((long) System.currentTimeMillis() / 1000);
				checkUser.save();
		...
		AppModel.getInstance()
				.getExceptionHandler()
				.setNewException(
						"Der eingegebene Benutzername oder das eingegebene Passwort ist inkorrekt! <p>Bitte versuche es noch einmal.</p>",
						"Fehler!");
		return null;
	}
