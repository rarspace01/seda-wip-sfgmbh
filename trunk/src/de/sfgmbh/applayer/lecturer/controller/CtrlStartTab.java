...
public class CtrlStartTab implements IntfCtrlStartTab {
	 ...
	 * 
	 * @see
	 * de.sfgmbh.applayer.lecturer.controller.IntfCtrlStartTab#saveCourse(de
	 * .sfgmbh.applayer.core.model.Course)
	 */
	@Override
	public boolean saveCourse(IntfCourse course) {
		...
		if (course.validate()) {
			...
			if (course.getLecturer_().getChair_() != null) {
				// Check if the logged in user is from the same chair as the
				// user he tries to save the course for
				if (loggedInUser.getChair_().getChairId_() == course
						.getLecturer_().getChair_().getChairId_()) {
					if (course.save()) {
						return true;
					}
				} else {
					exceptionHandler
							.setNewException(
									"Sie können nur Lehrveranstaltungen für Dozenten Ihres Lehrstuhls bearbeiten oder erstellen!",
									"Fehler!", "error");
				}
			} else {
				exceptionHandler
						.setNewException(
								"Lehrveranstaltungen können nur für Dozenten erstellt werden!",
								"Fehler!", "error");
			}
		}
		return false;
	 ...
	 * 
	 * @see
	 * de.sfgmbh.applayer.lecturer.controller.IntfCtrlStartTab#deleteCourse(
	 * de.sfgmbh.applayer.core.model.Course)
	 */
	@Override
	public boolean deleteCourse(IntfCourse course) {
		...
		// Check if the user for the course is a lecturer
		if (course.getLecturer_().getChair_() != null) {
			// Check if the logged in user is from the same chair as the user he
			// tries to save the course for
			if (loggedInUser.getChair_().getChairId_() == course.getLecturer_()
					.getChair_().getChairId_()) {
				return AppModel.getInstance().getRepositoryCourse()
						.delete(course);
			} else {
				exceptionHandler
						.setNewException(
								"Sie können nur Lehrveranstaltungen für Dozenten Ihres Lehrstuhls löschen!",
								"Fehler!", "error");
			}
		} else {
			exceptionHandler
					.setNewException(
							"Ein unerwarteter Fehler ist aufgetreten (die Veranstaltung konnte keinem Dozenten zugeordnet werden)!",
							"Fehler!", "error");
		}
	 ...
	 * 
	 * @see
	 * de.sfgmbh.applayer.lecturer.controller.IntfCtrlStartTab#suggest(de.sfgmbh
	 * .applayer.core.model.RoomAllocation, java.util.HashMap)
	 */
	@Override
	public IntfRoomAllocation suggest(IntfRoomAllocation roomAllocation,
		...
		List<IntfRoom> matchingRooms = AppModel.getInstance()
				.getRepositoryRoom().getByFilter(filter);
	 ...
	 * 
	 * @see
	 * de.sfgmbh.applayer.lecturer.controller.IntfCtrlStartTab#createRoomAllocation
	...
	public boolean createRoomAllocation(IntfRoomAllocation roomAllocation) {
		...
		if (roomAllocation.getTime_() < 1 || roomAllocation.getTime_() > 7
				|| roomAllocation.getDay_() < 1 || roomAllocation.getDay_() > 7
				|| roomAllocation.getSemester_() == null
				|| roomAllocation.getSemester_().equals("")) {
			exceptionHandler.setNewException(
					"Bitte wählen Sie ein gültiges Datum!", "Fehler!", "error");
			return false;
		}
		...
		if (roomAllocation.getCourse_().getLecturer_().getChair_() != null) {
			// Check if the logged in user is from the same chair as the user he
			// tries to create an allocation for
			if (loggedInUser.getChair_().getChairId_() == roomAllocation
					.getCourse_().getLecturer_().getChair_().getChairId_()) {
				...
				for (IntfRoomAllocation conflictRoomAllocations : roomAllocation
					...
					if (conflictRoomAllocations.getApproved_()
							...
							|| conflictRoomAllocations.getApproved_().equals(
									"denied")) {
						if (conflictRoomAllocations.getCourse_().getCourseId_() != roomAllocation
								.getCourse_().getCourseId_()) {
							allowSave = true;
						}
					}
				}
				...
				exceptionHandler
						.setNewException(
								"Sie können nur Lehrveranstaltungen für Dozenten Ihres Lehrstuhls anlegen!",
								"Fehler!", "error");
			}
		} else {
			exceptionHandler
	 ...
	 * 
	 * @see
	 * de.sfgmbh.applayer.lecturer.controller.IntfCtrlStartTab#revokeRoomAllocation
	 * (de.sfgmbh.applayer.core.model.RoomAllocation)
	 */
	@Override
	public boolean revokeRoomAllocation(IntfRoomAllocation roomAllocation) {
		...
		if (roomAllocation.getCourse_().getLecturer_().getChair_() != null) {
			...
			if (loggedInUser.getChair_().getChairId_() == roomAllocation
				...
				exceptionHandler
						.setNewException(
								"Sie können nur Lehrveranstaltungen für Dozenten Ihres Lehrstuhls zurückziehen!",
								"Fehler!", "error");
			}
		} else {
			exceptionHandler
	 ...
	 * 
	 * @see
	 * de.sfgmbh.applayer.lecturer.controller.IntfCtrlStartTab#counterRoomAllocation
	 * (de.sfgmbh.applayer.core.model.RoomAllocation, boolean)
	 */
	@Override
	public boolean counterRoomAllocation(IntfRoomAllocation roomAllocation,
			boolean accept) {
		AppException exceptionHandler = AppModel.getInstance()
				.getExceptionHandler();
		IntfUser loggedInUser = SessionManager.getInstance().getSession();
		...
		roomAllocation = AppModel.getInstance().getRepositoryRoomAllocation()
	 ...
	 * (non-Javadoc)
	 * 
	 * @see de.sfgmbh.applayer.lecturer.definitions.IntfCtrlStartTab#
	 * hasBeenSeenAllocation
