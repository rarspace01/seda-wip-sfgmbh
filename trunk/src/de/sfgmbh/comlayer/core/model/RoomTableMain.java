...
public class RoomTableMain extends DefaultTableModel implements IntfAppObserver {
	...
	private String[] header_ = { "Raum", "Gebäude", "Stock.", "Plätze",
			"PC-Plätze", "Beamer", "Visualizer", "Overheads", "Tafeln",
			"Whiteboards", "Hidden" };
	...
	private void change(String variant) {
		...
		if (variant.equals("init")) {
			...
			String seats;
			try {
				int intSeats = Integer.parseInt(baseTab
						.getComboBoxSeatsFilter().getSelectedItem().toString());
				seats = String.valueOf(intSeats);
			} catch (Exception e) {
				AppModel.getInstance()
						.getExceptionHandler()
						.setNewException(
								"Bitte stellen Sie sicher,  dass Sie eine Zahlen bei dem Sitze-Filtern eingetragen haben.",
								"Fehler!", "error");
				baseTab.getComboBoxSeatsFilter().setSelectedItem("0");
				return;
			}
			filter.put("room", baseTab.getComboBoxRoomnumberFilter()
					.getSelectedItem().toString());
			filter.put("seats", seats);
			filter.put("level", baseTab.getComboBoxLevelFilter()
					.getSelectedItem().toString());
		}
		...
		for (IntfRoom room : AppModel.getInstance().getRepositoryRoom()
			...
			// if (room.isPublic()) {
			try {
				Object[] row = { room.getRoomNumber_(), room.getBuildingId_(),
						room.getLevel_(), room.getSeats_(), room.getPcseats_(),
						room.getBeamer_(), room.getVisualizer_(),
						room.getOverheads_(), room.getChalkboards_(),
						room.getWhiteboards_(), room, };
				this.addRow(row);
