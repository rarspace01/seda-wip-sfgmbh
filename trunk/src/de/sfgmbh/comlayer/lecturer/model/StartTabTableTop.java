 ...
 * 
 */
public class StartTabTableTop extends DefaultTableModel implements
		IntfAppObserver {

	private static final long serialVersionUID = 1L;
	private String[] header_ = { "Bezeichnung", "Art", "Dozent", "SWS",
			"Erw. Teilnehmer", "öffentlich", "Hidden" };
		...
		if (sessionUser.getChair_() != null) {
			...
			if (variant.equals("init")) {
				...
				filter.put("lecturer", ViewManager.getInstance()
						.getLecturerStartTab().getComboLecturer()
			...
			for (IntfCourse course : AppModel.getInstance()
				...
				try {
					Object[] row = {
							...
							ViewHelper.getTextForBoolean(course
									.isLecturerEnabled_()), course };
					this.addRow(row);
