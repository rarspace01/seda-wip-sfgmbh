...
public class UserTabBtnsControl implements ActionListener,
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (this.navAction_.equals("edit")) {
			// Get the user
			int row = ViewManager.getInstance().getOrgaUserTab()
					.getUserOrgaTable().getSelectedRow();
			if (row == -1) {
				AppModel.getInstance()
						.getExceptionHandler()
						.setNewException(
								"Sie müssen zunächst einen Benutzer auswählen.",
								"Achtung!");
			} else {
				try {
					row = ViewManager.getInstance().getOrgaUserTab()
							.getRowSorter().convertRowIndexToModel(row);
					IntfUser editUser = (IntfUser) ViewManager.getInstance()
							.getOrgaUserTableModel().getValueAt(row, 6);
		...
		if (this.navAction_.equals("delete")) {
			// Set a user delete variable to be sure the user really pressed the
			// delete button later
			deleteUser_ = true;
			...
			if (row == -1) {
				...
				try {
					row = ViewManager.getInstance().getOrgaUserTab()
							.getRowSorter().convertRowIndexToModel(row);
					this.userMarkedForDeletion_ = (IntfUser) ViewManager
							.getInstance().getOrgaUserTableModel()
							.getValueAt(row, 6);
				} catch (Exception ex) {
					AppModel.getInstance()
							.getExceptionHandler()
				...
				QuestionDialog dialog = new QuestionDialog(
						"Wollen Sie den gewählten Benutzer wirklich löschen?",
						"Achtung!");
